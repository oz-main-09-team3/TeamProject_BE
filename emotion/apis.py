...
class EmotionTrendView(APIView):
    def get(self, request):
        start = request.query_params.get('from') # query_params.get(): URL의 쿼리 파라미터 ?from=...&to=...를 가져옴
        ...
        data = EmotionData.objects.filter(created_at__gte=start_dt, created_at__lt=end_dt)
        response = EmotionTrendViewSerializer(data, many=True).data # EmotionTrendViewSerializer로 JSON 형태로 직렬화
...
class EmotionCountView(APIView):
    def get(self, request):
        ...
        logs = EmotionData.objects.filter(created_at__gte=start_dt, created_at__lt=end_dt)
        ...
        result = [{'emotion': EmotionSerializer(e).data, 'count': count} for e, count in counts.items()]
